        ...
        if(player!=nullptr)
        {
            if(player->getBullet() && player->getFrame()>8)
            {
                ...
                fire = true;
                    ...
                    {
                    case LEFT:
                        bulletSprite.setPosition(player->getSprite().getPosition().x+30,player->getSprite().getPosition().y-10);
                        bulletDirection = LEFT; break;
                    case RIGHT:
                        bulletSprite.setPosition(player->getSprite().getPosition().x+30,player->getSprite().getPosition().y-20);
                        bulletDirection = RIGHT; break;
                    case DOWN:
                        bulletSprite.setPosition(player->getSprite().getPosition().x,player->getSprite().getPosition().y+60);
                        bulletDirection = DOWN; break;
                    case UP:
                        bulletSprite.setPosition(player->getSprite().getPosition().x,player->getSprite().getPosition().y-60);
                        bulletDirection = UP; break;
                    }
            }
            if(fire&&rect.getGlobalBounds().intersects(bulletSprite.getGlobalBounds()))
            {
                ...
                {
                case LEFT:
                    bulletSprite.move(-25,0); bulletSprite.setRotation(0); break;
                case RIGHT:
                    bulletSprite.move(25,0); bulletSprite.setRotation(0); break;
                case DOWN:
                    bulletSprite.move(0,25);  bulletSprite.setRotation(90); break;
                case UP:
                    bulletSprite.move(0,-25); bulletSprite.setRotation(90); break;
                }
